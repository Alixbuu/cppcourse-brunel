...
// test membrane potential for different imput
...
// test when we give a negative current to the membrane potential
...
TEST (NeuronTest, SpikeTimes)
{
	...
	// also check that the value  of the membrane potential go back to 0.0 after a spike
    ...
    // checking the number of occuring spike
...
// test when neuron2 receive one spike from neuron1 that it doesnt spike
...
// test if the neuron2 spike after receiving two spikes from neuron 1
TEST (TwoNeuronTest,withspike2)
{
	...
	// we wait for the second spike and observe the impact on the second neuron --> neuron 2 should spikes
	// neuron 2 have to receive 2 spike from neuron 1 to pass the threshold , 1868+2 step because sendspike is called after the incrementation of time in update
	for( int i(0); i<1870+delay; ++i) 
	{
		if(neuron1.updatetest(I))
		{
		neuron1.sendspike(neuron2,1,i);
